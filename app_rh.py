...
def exportar_epi_word(df, caminho, unidade="", responsavel="", data_pedido="", cliente=""):
    ...
    # --- CABEÇALHO: Endereço (igual imagem) ---
    p = doc.add_paragraph()
    ...
    # --- TABELA DE ESTADOS (igual imagem) ---
    estados_linha1 = [
    ...
    texto_estados1 = "  ".join([f"{nome} ( {'x' if marcado else ' '} )" for nome, marcado in estados_linha1])
    run = p.add_run(texto_estados1)
    run.font.size = Pt(8)
    run.font.bold = True
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    texto_estados2 = "  ".join([f"{nome} ( {'x' if marcado else ' '} )" for nome, marcado in estados_linha2])
    run = p.add_run(texto_estados2)
    run.font.size = Pt(8)
    run.font.bold = True
    ...
    # Tabela Botas: 7 colunas (igual imagem)
    tbl_botas = doc.add_table(rows=1 + max(len(botas), 1), cols=7)
    ...
    if not botas.empty:
        ...
        # Pelo menos uma linha vazia
        for cell in tbl_botas.rows[1].cells:
            cell.text = ""
    ...
    # --- SEÇÃO EPIS ---
    # Lista fixa de EPIs conforme a imagem (16 itens, distribuídos em 3 colunas)
    epi_lista = [
    ...
    # Na imagem são 6 linhas de dados (máximo por coluna) + 1 cabeçalho
    max_linhas = 6  # máximo de itens na coluna 1
    tbl_epis = doc.add_table(rows=1 + max_linhas, cols=6)
    ...
    # Distribuir EPIs em 3 colunas: 6 + 5 + 5 = 16
    distribuicao = [6, 5, 5]
    idx_epi = 0
    for col_idx, qtd_col in enumerate(distribuicao):
        for row_offset in range(qtd_col):
            if idx_epi >= len(epi_lista):
                break
            epi = epi_lista[idx_epi]
            ...
            epi_upper = epi.upper().strip()
            qtd_val = ""
            for mat_key, qtd_str in materiais_pedido.items():
                # Matching mais flexível
                mat_key_clean = mat_key.replace(" ", "").replace("_", "").replace("-", "").replace(".", "").replace(",", "")
                epi_clean = epi_upper.replace(" ", "").replace("_", "").replace("-", "").replace(".", "").replace(",", "").replace("(", "").replace(")", "")
                
                # Verificar se um contém o outro
                if epi_clean in mat_key_clean or mat_key_clean in epi_clean:
                    qtd_val = qtd_str
                    break
    ...
    for _, r in df.iterrows():
        anexo_path = str(r.get("ANEXO", "")).strip()
        if anexo_path and os.path.exists(anexo_path) and anexo_path not in anexos:
            anexos.append(anexo_path)
    
    if anexos:
        doc.add_page_break()
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run("ANEXOS")
        run.font.size = Pt(12)
        run.font.bold = True
        doc.add_paragraph()
        
        for idx, anexo_path in enumerate(anexos, 1):
            ext = os.path.splitext(anexo_path)[1].lower()
            nome_arq = os.path.basename(anexo_path)
            
            p = doc.add_paragraph()
            run = p.add_run(f"Anexo {idx}: {nome_arq}")
            run.font.size = Pt(10)
            run.font.bold = True
            
            if ext in (".jpg", ".jpeg", ".png"):
                try:
                    doc.add_picture(anexo_path, width=Cm(14))
                    last_paragraph = doc.paragraphs[-1]
                    last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
                except Exception:
                    p = doc.add_paragraph()
                    run = p.add_run("[Erro ao inserir imagem]")
                    run.font.size = Pt(9)
                    run.font.color.rgb = RGBColor(255, 0, 0)
            elif ext == ".pdf":
                p = doc.add_paragraph()
                run = p.add_run("[Documento PDF - verificar arquivo anexado separadamente]")
                run.font.size = Pt(9)
