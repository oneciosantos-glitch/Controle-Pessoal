...
def lista_clientes_compras(dados_c=None):
    ...
    return sorted([c for c in df["Nome"].astype(str).str.strip().unique() if c and c != "nan"])
...
def lista_lojas_compras(dados_c=None, cliente=None):
    ...
    return sorted([l for l in lojas["Nome"].astype(str).str.strip().unique() if l and l != "nan"])
...
def lista_materiais_nomes(dados_c=None, cliente=None):
    ...
    return sorted([m for m in mats["Nome"].astype(str).str.strip().unique() if m and m != "nan"])
...
with aba11:
    ...
    with sub_aba_nova:
        ...
        with st.form("form_nova_solicitacao", clear_on_submit=False):
            ...
            with col1:
                tipo_sol = st.selectbox("Tipo *", TIPOS_SOLICITACAO)
                cliente_sol = st.selectbox("Cliente *", [""] + lista_clientes_compras(dados_c))
            with col2:
                lojas_disp = [""] + lista_lojas_compras(dados_c, cliente_sol) if cliente_sol else [""]
                loja_sol = st.selectbox("Loja *", lojas_disp)
            ...
            if tipo_sol == "Material":
                ...
                if mats_disp == [""]:
                    st.info("Cadastre materiais no catálogo para selecionar.")
                ...
                mats_epi = ["Camisa Polo", "Calça", "Botina", "Capacete", "Óculos", "Luva", "Máscara", "Protetor Auricular", "Colete Refletivo"]
