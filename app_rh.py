...
CATEGORIAS_DESPESA = ["Alimentação", "Transporte", "Hospedagem", "Estacionamento", "Pedágio", "Combustível", "Outros"]
SITUACOES_VIAGEM = ["Ativa", "Concluída", "Cancelada"]

PADRAO_VIAGENS = {
    "ID_Viagem": "", "Funcionario": "", "Destino": "", "DataIda": "", "DataVolta": "",
    "Motivo": "", "ValorRecebido": 0.0, "TotalDespesas": 0.0, "Saldo": 0.0, "Situacao": "Ativa"
}

PADRAO_PRESTACAO = {
    "ID_Viagem": "", "DataDespesa": "", "Categoria": "", "Descricao": "", "Valor": 0.0, "Comprovante": ""
}

def _garantir_colunas_viagem(df, padrao):
    for col, val in padrao.items():
        if col not in df.columns:
            df[col] = val
    return df

def carregar_aba_viagem(nome_aba, padrao):
    try:
        df = pd.read_excel(ARQUIVO, sheet_name=nome_aba, dtype=str, keep_default_na=False)
        df = _garantir_colunas_viagem(df, padrao)
        for col in ["ValorRecebido", "TotalDespesas", "Saldo", "Valor"]:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col].astype(str).str.replace(",", "."), errors="coerce").fillna(0.0)
        return df
    except Exception:
        return pd.DataFrame(columns=list(padrao.keys()))

def salvar_viagens_e_prestacao(df_viagens, df_prestacao):
    try:
        todas = pd.read_excel(ARQUIVO, sheet_name=None, dtype=str, keep_default_na=False)
    except Exception:
        todas = {}
    
    todas["Viagens"] = df_viagens.copy()
    todas["PrestacaoContas"] = df_prestacao.copy()
    
    for nome, df in list(todas.items()):
        for col in df.columns:
            if col in ["ValorRecebido", "TotalDespesas", "Saldo", "Valor"]:
...
def gerar_id_viagem(df):
    if df.empty or "ID_Viagem" not in df.columns:
        return "V001"
    try:
        nums = df["ID_Viagem"].astype(str).str.extract(r"V(\d+)")[0].dropna().astype(int)
        return f"V{nums.max() + 1:03d}"
    except Exception:
        return "V001"

def atualizar_totais_viagem(id_viagem, df_viagens, df_prestacao):
    if id_viagem not in df_viagens["ID_Viagem"].values:
        return df_viagens
    total = df_prestacao[df_prestacao["ID_Viagem"] == id_viagem]["Valor"].sum()
    idx = df_viagens[df_viagens["ID_Viagem"] == id_viagem].index[0]
    df_viagens.at[idx, "TotalDespesas"] = total
    recebido = float(df_viagens.at[idx, "ValorRecebido"]) if pd.notna(df_viagens.at[idx, "ValorRecebido"]) else 0.0
    df_viagens.at[idx, "Saldo"] = recebido - total
    return df_viagens

def lista_funcionarios_viagem():
    try:
        df = pd.read_excel(ARQUIVO, sheet_name="Base_Dados", dtype=str, keep_default_na=False)
        nomes = df["Nome"].dropna().astype(str).str.strip().unique().tolist()
...
def aba_guia_viagem():
    ...
    df_viagens = carregar_aba_viagem("Viagens", PADRAO_VIAGENS)
    df_prestacao = carregar_aba_viagem("PrestacaoContas", PADRAO_PRESTACAO)

    tab1, tab2 = st.tabs(["✈️ Cadastrar Viagem", "📋 Viagens e Prestação de Contas"])

    with tab1:
        st.markdown("#### Nova Viagem")
        with st.form("form_viagem", clear_on_submit=True):
            c1, c2, c3 = st.columns(3)
            with c1:
                func_opts = lista_funcionarios_viagem()
                func_sel = st.selectbox("Funcionário", func_opts, index=0 if func_opts else None)
                destino = st.text_input("Destino")
            with c2:
                dt_ida = st.date_input("Data Ida", value=datetime.now())
                dt_volta = st.date_input("Data Volta", value=datetime.now())
            with c3:
                motivo = st.text_input("Motivo")
                val_rec = st.number_input("Valor Recebido (R$)", min_value=0.0, format="%.2f")

            submitted = st.form_submit_button("💾 Salvar Viagem")
            if submitted:
                if not func_sel or not destino:
                    st.error("Preencha Funcionário e Destino.")
                else:
                    novo_id = gerar_id_viagem(df_viagens)
                    nova = pd.DataFrame([{
                        "ID_Viagem": novo_id,
                        "Funcionario": func_sel,
                        "Destino": destino,
                        "DataIda": dt_ida.strftime("%d/%m/%Y"),
                        "DataVolta": dt_volta.strftime("%d/%m/%Y"),
                        "Motivo": motivo,
                        "ValorRecebido": val_rec,
                        "TotalDespesas": 0.0,
                        "Saldo": val_rec,
                        "Situacao": "Ativa"
                    }])
                    df_viagens = pd.concat([df_viagens, nova], ignore_index=True)
                    salvar_viagens_e_prestacao(df_viagens, df_prestacao)
                    st.success(f"Viagem {novo_id} cadastrada!")
                    st.rerun()
    ...
    with tab2:
        ...
        with fc1:
            filtro_func = st.selectbox("Funcionário", ["Todos"] + lista_funcionarios_viagem(), key="filtro_func_v")
        with fc2:
            filtro_dest = st.text_input("Destino", key="filtro_dest_v")
        with fc3:
            try:
                min_d = datetime.strptime(df_viagens["DataIda"].astype(str).str.strip().replace("", "01/01/2000").min(), "%d/%m/%Y") if not df_viagens.empty else datetime(2000, 1, 1)
                max_d = datetime.strptime(df_viagens["DataIda"].astype(str).str.strip().replace("", "31/12/2099").max(), "%d/%m/%Y") if not df_viagens.empty else datetime(2099, 12, 31)
            except Exception:
                min_d, max_d = datetime(2000, 1, 1), datetime(2099, 12, 31)
            dt_ini = st.date_input("De", value=min_d, key="dt_ini_v")
        with fc4:
            dt_fim = st.date_input("Até", value=max_d, key="dt_fim_v")

        filtro_sit = st.selectbox("Situação", ["Todas"] + SITUACOES_VIAGEM, key="filtro_sit_v")

        lista_v = df_viagens.copy()
        if filtro_func != "Todos":
            lista_v = lista_v[lista_v["Funcionario"].astype(str).str.strip() == filtro_func]
        if filtro_dest.strip():
            lista_v = lista_v[lista_v["Destino"].astype(str).str.contains(filtro_dest, case=False, na=False)]
        if filtro_sit != "Todas":
            lista_v = lista_v[lista_v["Situacao"].astype(str).str.strip() == filtro_sit]
        ...
        if not lista_v.empty:
            lista_v = lista_v.copy()
            lista_v["_dt_ida"] = lista_v["DataIda"].apply(_parse_dt)
            lista_v = lista_v[(lista_v["_dt_ida"] >= datetime(dt_ini.year, dt_ini.month, dt_ini.day)) & (lista_v["_dt_ida"] <= datetime(dt_fim.year, dt_fim.month, dt_fim.day))]
            lista_v = lista_v.drop(columns=["_dt_ida"], errors="ignore")

        st.markdown("---")
        st.markdown(f"**{len(lista_v)} viagem(ns) encontrada(s)**")

        if lista_v.empty:
            st.info("Nenhuma viagem encontrada com os filtros aplicados.")
        else:
            for _, row in lista_v.iterrows():
                idv = str(row["ID_Viagem"]).strip()
                saldo_val = float(row["Saldo"]) if pd.notna(row["Saldo"]) else 0.0
                cor_saldo = "green" if saldo_val >= 0 else "red"
                icone_saldo = "✅" if saldo_val >= 0 else "⚠️"

                with st.expander(f"🧳 {idv} | {row['Funcionario']} → {row['Destino']} | Situação: {row['Situacao']}"):
                    col_a, col_b, col_c = st.columns([2, 2, 1])
                    with col_a:
                        st.write(f"**Datas:** {row['DataIda']} a {row['DataVolta']}")
                        st.write(f"**Motivo:** {row['Motivo']}")
                    with col_b:
                        st.write(f"**Valor Recebido:** R$ {float(row['ValorRecebido']):,.2f}")
                        st.write(f"**Total Despesas:** R$ {float(row['TotalDespesas']):,.2f}")
                    with col_c:
                        st.markdown(f"<h4 style='color:{cor_saldo};margin:0'>{icone_saldo} Saldo: R$ {saldo_val:,.2f}</h4>", unsafe_allow_html=True)

                        nova_sit = st.selectbox("Alterar Situação", SITUACOES_VIAGEM, index=SITUACOES_VIAGEM.index(row["Situacao"]) if row["Situacao"] in SITUACOES_VIAGEM else 0, key=f"sit_{idv}")
                        if nova_sit != row["Situacao"]:
                            idx = df_viagens[df_viagens["ID_Viagem"] == idv].index[0]
                            df_viagens.at[idx, "Situacao"] = nova_sit
                            salvar_viagens_e_prestacao(df_viagens, df_prestacao)
                    ...
                    with st.form(f"form_despesa_{idv}", clear_on_submit=True):
                        d1, d2, d3 = st.columns(3)
                        with d1:
                            dt_desp = st.date_input("Data Despesa", value=datetime.now(), key=f"dt_d_{idv}")
                            cat = st.selectbox("Categoria", CATEGORIAS_DESPESA, key=f"cat_{idv}")
                        with d2:
                            desc = st.text_input("Descrição", key=f"desc_{idv}")
                            val_desp = st.number_input("Valor (R$)", min_value=0.0, format="%.2f", key=f"val_{idv}")
                        with d3:
                            comp = st.file_uploader("Comprovante", type=["jpg", "jpeg", "png", "pdf"], key=f"comp_{idv}")

                        add_desp = st.form_submit_button("➕ Adicionar Despesa")
                        if add_desp:
                            caminho_comp = ""
                            if comp is not None:
                                ext = comp.name.split(".")[-1]
                                nome_arq = f"{idv}_{datetime.now().strftime('%Y%m%d%H%M%S')}.{ext}"
                                caminho_comp = os.path.join(PASTA_COMPROVANTES, nome_arq)
                                with open(caminho_comp, "wb") as f:
                                    f.write(comp.getbuffer())

                            nova_desp = pd.DataFrame([{
                                "ID_Viagem": idv,
                                "DataDespesa": dt_desp.strftime("%d/%m/%Y"),
                                "Categoria": cat,
                                "Descricao": desc,
                                "Valor": val_desp,
                                "Comprovante": caminho_comp
                            }])
                            df_prestacao = pd.concat([df_prestacao, nova_desp], ignore_index=True)
                            df_viagens = atualizar_totais_viagem(idv, df_viagens, df_prestacao)
                            salvar_viagens_e_prestacao(df_viagens, df_prestacao)
                            st.success("Despesa adicionada!")
                            st.rerun()

                    despesas = df_prestacao[df_prestacao["ID_Viagem"].astype(str).str.strip() == idv].copy()
                    if not despesas.empty:
                        st.markdown("##### 📄 Despesas Registradas")
                        st.dataframe(despesas[["DataDespesa", "Categoria", "Descricao", "Valor", "Comprovante"]], use_container_width=True, hide_index=True)
                    else:
...
aba1, aba2, aba3, aba4, aba5, aba6, aba7, aba8 = st.tabs([
    "Cadastro", "Painel", "Prazos e Férias", "Histórico", "Relatórios", "📎 Documentos", "⚙️ Lojas e Cargos", "🧳 Guia de Viagem"
])
